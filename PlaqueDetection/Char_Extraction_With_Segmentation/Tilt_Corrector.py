...
class Rotator:
    ...
    def find_bottom_coordinates(self):
        ...
        # Previous and next coordinates in the array
        previous_cor = 3 if idx == 0 else idx - 1
        next_cor = 0 if idx == 3 else idx + 1

        # Distances between the acquired coordinate and its previous and next coordinates
        ...
        # Determine the second bottom-most coordinate based on the distance
        if p > n:
    ...
    def rotate_image(self):
        ...
        """
        left, right = self.find_bottom_coordinates()  # Get bottom-left and bottom-right coordinates
        left_x, left_y = left[0], left[1]
        right_x, right_y = right[0], right[1]
        print(f"Bottom-left coordinate: ({left_x}, {left_y}), Bottom-right coordinate: ({right_x}, {right_y})")
        ...
        image_center = tuple(np.array(Cropped_LP.shape[1::-1]) / 2)  # Center of the image
        rot_mat = cv2.getRotationMatrix2D(image_center, theta, 1.0)  # Rotation matrix
        result = cv2.warpAffine(Cropped_LP, rot_mat, Cropped_LP.shape[1::-1], flags=cv2.INTER_LINEAR)  # Apply rotation

        # Crop the image after rotation
        opp = right_y - left_y
        h = result.shape[0] - opp // 2 if opp > 0 else result.shape[0] + opp // 2
        result = result[0:h, :]

        # Display the rotated plate
        plt.imshow(result)
...
#plate_bounding_box = [[910, 520], [1104, 520], [1104, 577], [910, 577]] # Bounding box example detected by YOLO
#cropped_path = ""
#rotator = Rotator(plate_bounding_box, cropped_path)
#rotated_plate = rotator.rotate_image()
#rotated_path=""
#cv2.imwrite(rotated_path, rotated_plate)
