...
export interface CarModeProps {
  chartSeries: number[];
  labels: string[];
  sx?: SxProps;
}

export function CarMode({ chartSeries, labels, sx }: CarModeProps): React.JSX.Element {
  const chartOptions = useChartOptions(labels);

  return (
    <Card sx={sx}>
      <CardHeader title="Car mode" />
      <CardContent>
        <Stack spacing={2}>
          <Chart height={230} options={chartOptions} series={chartSeries} type="donut" width="100%" />
